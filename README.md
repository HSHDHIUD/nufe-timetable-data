...
- 全编号扫描无课表数据：24 个早期学期
- 班级课程实例：97,314 条
- 班级周课表：12,894 个班级页面，覆盖全部 33 个有数据学期
- 专业：146 个
- 历史教师：2,372 位
- 教师授课历史：111,861 条
...
- 各学期行政班/专业课表还原结果
- 全部 33 个有数据学期的班级周课表 HTML（`2008-2009-1` 仅有残留活动，无可归属行政班）
- 全历史专业课程矩阵与最近两年课程矩阵
- 教师总表和教师授课历史
- 本项目使用的数据处理脚本
...
公开教室课表接口不直接提供行政班所属学院；课程记录中的开课学院不能等同于班级所属学院。教师所属学院在绝大多数教师记录中可直接取得，但当前数据没有教师性别字段，本项目不按姓名推断性别。
